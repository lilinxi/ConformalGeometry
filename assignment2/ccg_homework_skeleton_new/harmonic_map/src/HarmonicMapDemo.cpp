...
void MeshLib::CHarmonicMap::_calculate_edge_weight() 
{
    ...
    for (M::MeshFaceIterator fiter(m_pMesh); !fiter.end(); ++fiter)
    {
        ...
        int i = 0;
        for(M::FaceHalfedgeIterator eIter(pF); !eIter.end(); ++eIter) {
            pH[i] = dynamic_cast<M::CHalfEdge*>(*eIter);
            assert(pH[i]);
            i++;
        }
        assert(pH[1]->edge() && pH[2]->edge() && pH[0]->edge());
        pH[0]->angle() = _inverse_cosine_law(dynamic_cast<M::CEdge*>(pH[1]->edge())->length(), dynamic_cast<M::CEdge*>(pH[2]->edge())->length(), dynamic_cast<M::CEdge*>(pH[0]->edge())->length());
        pH[1]->angle() = _inverse_cosine_law(m_pMesh->halfedgeEdge(pH[0])->length(), m_pMesh->halfedgeEdge(pH[2])->length(), m_pMesh->halfedgeEdge(pH[1])->length());
        pH[2]->angle() = _inverse_cosine_law(m_pMesh->halfedgeEdge(pH[1])->length(), m_pMesh->halfedgeEdge(pH[0])->length(), m_pMesh->halfedgeEdge(pH[2])->length());
    }
    ...
    for (M::MeshEdgeIterator eiter(m_pMesh); !eiter.end(); ++eiter)
    {
        ...
        if(pE->boundary()) { 
            MeshLib::CHalfEdge* half_edge = nullptr;
            half_edge = pE->halfedge(0) ? pE->halfedge(0): pE->halfedge(1);
           pE->weight() = 1.0 / std::tan(dynamic_cast<M::CHalfEdge*>(half_edge)->angle());
        } else {
            pE->weight() = 1.0 / std::tan(dynamic_cast<M::CHalfEdge*>(pE->halfedge(0))->angle());
            pE->weight() += 1.0 / std::tan(dynamic_cast<M::CHalfEdge*>(pE->halfedge(1))->angle());
        }
    }
}
