...
class COperator
{
  public:
    ...
    /*!
     *	set vertex default color; if vertex has color, then keep the original color
     *	otherwise, set the default color
     */
    void _set_vertex_default_color();
  ...
  protected:
     ...
     */
    int
    _circle_circle_intersection(double x0, double y0, double r0, double x1, double y1, double r1, double* xi, double* yi, double* xi_prime, double* yi_prime);
};
...
void COperator<M>::_metric_2_angle()
{
    ...
    for (typename M::MeshFaceIterator fiter(m_pMesh); !fiter.end(); fiter++)
    {
        typename M::CFace* f = *fiter;

        typename M::CHalfEdge* he[3];
        ...
        for (int i = 0; i < 3; i++)
        {
            typename M::CEdge* e = m_pMesh->halfedgeEdge(he[i]);
            l[i] = e->length();
        }
...
void COperator<M>::_angle_2_curvature()
{
    ...
    {
        typename M::CVertex* v = *viter;
        double k = (v->boundary()) ? PI : PI * 2;
        for (typename M::VertexInHalfedgeIterator vh(m_pMesh, v); !vh.end(); ++vh)
        {
            typename M::CHalfEdge* he = *vh;
            k -= he->angle();
        }
        v->k() = k;
    }
...
void COperator<M>::_angle_2_Laplace()
{
    ...
    for (typename M::MeshEdgeIterator eiter(m_pMesh); !eiter.end(); eiter++)
    {
        ...
        typename M::CHalfEdge* pNh = m_pMesh->faceNextCcwHalfEdge(he);

        double wt = cos(pNh->angle()) / sin(pNh->angle());

        typename M::CHalfEdge* sh = m_pMesh->halfedgeSym(he);
        if (sh != NULL)
        {
...
void COperator<M>::_parameter_mu_2_angle()
{
    for (typename M::MeshFaceIterator fiter(m_pMesh); !fiter.end(); fiter++)
    {
        ...
        typename M::CHalfEdge* hs[3];
        int i = 0;
        for (typename M::FaceHalfedgeIterator fhiter(f); !fhiter.end(); fhiter++)
        {
        ...
        for (int i = 0; i < 3; i++)
        {
            typename M::CHalfEdge* h = hs[i];
            typename M::CVertex* ps = m_pMesh->halfedgeSource(h);
            typename M::CVertex* pt = m_pMesh->halfedgeTarget(h);
...
void COperator<M>::_parameter_2_mu()
{
    for (typename M::MeshFaceIterator fiter(m_pMesh); !fiter.end(); fiter++)
    {
        ...
        int i = 0;
        typename M::CVertex* vs[3];
        for (typename M::FaceVertexIterator fviter(f); !fviter.end(); fviter++)
        {
...
std::complex<double> COperator<M>::_diagonal_ratio(M* pM, typename M::CEdge* pE)
{
    ...
    if (pM->isBoundary(pE))
    {
        ...
        typename M::CHalfEdge* pH = pM->edgeHalfedge(pE, 0);
        typename M::CHalfEdge* pN = pM->faceNextCcwHalfEdge(pH);

        typename M::CVertex* pS = pM->halfedgeSource(pH);
        typename M::CVertex* pT = pM->halfedgeTarget(pH);
        typename M::CVertex* pL = pM->halfedgeTarget(pN);
    ...
    else
    {
        ...
        typename M::CHalfEdge* pH = pM->edgeHalfedge(pE, 0);
        typename M::CHalfEdge* pN = pM->faceNextCcwHalfEdge(pH);
        typename M::CHalfEdge* pD = pM->edgeHalfedge(pE, 1);
        typename M::CHalfEdge* pW = pM->faceNextCcwHalfEdge(pD);

        typename M::CVertex* pS = pM->halfedgeSource(pH);
        typename M::CVertex* pT = pM->halfedgeTarget(pH);
        typename M::CVertex* pL = pM->halfedgeTarget(pN);
        typename M::CVertex* pR = pM->halfedgeTarget(pW);
...
bool COperator<M>::_embed_third_vertex(M* pM, typename M::CVertex* A, typename M::CVertex* B, typename M::CVertex* C)
{
    ...
    int result = _circle_circle_intersection(c1[0], c1[1], r1, c2[0], c2[1], r2, &i1[0], &i1[1], &i2[0], &i2[1]);
...
template <typename M>
int COperator<M>::_circle_circle_intersection(
    double x0, double y0, double r0, double x1, double y1, double r1, double* xi, double* yi, double* xi_prime, double* yi_prime)
{
...
void COperator<M>::_calculate_face_vertex_normal()
{
    ...
    for (typename M::MeshFaceIterator fiter(m_pMesh); !fiter.end(); ++fiter)
    {
        ...
        for (typename M::FaceVertexIterator fviter(pF); !fviter.end(); fviter++)
        {
            typename M::CVertex* pV = *fviter;
            p.push_back(pV->point());
        }
    ...
    for (typename M::MeshVertexIterator viter(m_pMesh); !viter.end(); ++viter)
    {
        ...
        for (typename M::VertexFaceIterator vfiter(v); !vfiter.end(); ++vfiter)
        {
            typename M::CFace* pF = *vfiter;
            n += pF->normal();
        }
        n = n / n.norm();
        v->normal() = n;
...
void COperator<M>::_uv_2_pos()
{
    for (typename M::MeshVertexIterator viter(m_pMesh); !viter.end(); ++viter)
    {
        typename M::CVertex* v = *viter;
        CPoint2 uv = v->uv();
        v->Point() = CPoint(uv[0], uv[1], 0);
    }
};

template <typename M>
void COperator<M>::_set_vertex_default_color()
{
    bool has_rgb = false;
    for (typename M::MeshVertexIterator viter(m_pMesh); !viter.end(); viter++)
    {
